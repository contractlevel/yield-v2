...
/// @notice ChildVault TVL with no active adapter counts only rebalance CCIP-send recovery
...
///      s_ccipSendRecovery.amount if that rebalance send failed, while epoch withdrawals are excluded, so TVL is
...
rule ADAPTER_003_ADAPTER_005_REBAL_008_REC_006_getTVL_EqualsCcipSendRecovery_WhenNoActiveAdapter() {
    ...
    assert getTVL() == (getCcipSendRecoveryTxType() == Types.CcipTx.REBALANCE
        ? getCcipSendRecoveryAmount() : 0);
}
...
rule ADAPTER_003_ADAPTER_005_REBAL_008_REC_006_getTVL_IncludesAdapterTVLAndRecoveries() {
    ...
    uint256 ccipSendRecoveryAmount = getCcipSendRecoveryTxType() == Types.CcipTx.REBALANCE
        ? getCcipSendRecoveryAmount() : 0;
...
rule tryCcipSend_RevertWhen_CallerIsNotSelf() {
    ...
    /// @dev Exclude the permanent capacity error from ordinary send behavior.
    require !ccipRouter.capacityError(), "router error is not a capacity error";
...
/// @notice A capacity error during CCIP recovery preserves the original recovery atomically.
/// @dev Verifies recovery clearing, token approvals, and events roll back.
rule CCIP_007_REC_005_executeRecovery_CCIP_SEND_RevertWhen_RouterCcipSendCapacityExceeded() {
    env e;
