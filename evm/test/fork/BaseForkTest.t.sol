...
abstract contract BaseForkTest is BaseDeploymentTest {
    ...
    function _createForks() internal {
        baseFork = vm.createSelectFork(vm.envString("BASE_MAINNET_RPC_URL"), BASE_FORK_BLOCK);
        arbitrumFork = vm.createFork(vm.envString("ARBITRUM_MAINNET_RPC_URL"), ARBITRUM_FORK_BLOCK);
        ...
        polygonFork = vm.createFork(vm.envString("POLYGON_MAINNET_RPC_URL"), POLYGON_FORK_BLOCK);
    }
    ...
    function _deployForkTopology() internal {
        ...
        polygonConfig = configHelper.getPolygonConfig();
        ...
        _setForkNetworkDetails(polygonConfig);
        networkConfig = baseConfig;
        _assertNetworkDetails(baseConfig);

        DeployParent parentDeployer = new DeployParent();
        parentForkDeployer = address(parentDeployer);
        parent = _parentFromDeployment(parentDeployer.deployWithConfig(baseConfig, parentForkDeployer));
        _makeParentPersistent(parent);
        _labelParentIntegrationContracts();

        (arbitrumChild, arbitrumForkDeployer) = _deployForkChild(arbitrumFork, arbitrumConfig);
        ...
        (polygonChild, polygonForkDeployer) = _deployForkChild(polygonFork, polygonConfig);
    ...
    function _configureForkCrosschainVaults() internal {
        ...
        _setCrosschainVault(parent.vault, polygonConfig.ccip.thisChainSelector, address(polygonChild.vault));

        _configureChildCrosschainVault(arbitrumFork, arbitrumConfig, arbitrumChild);
        ...
        _configureChildCrosschainVault(polygonFork, polygonConfig, polygonChild);
    }
    ...
    function _configureChildCrosschainVault(
        ...
        vm.selectFork(forkId);
        networkConfig = config;
        _setCrosschainVault(forkChild.vault, baseConfig.ccip.thisChainSelector, address(parent.vault));
    }
    ...
    function _chainIdFromSelector(uint64 chainSelector) internal pure returns (uint256) {
        ...
        if (chainSelector == 4051577828743386545) return POLYGON_CHAIN_ID;
    ...
    function _selectPolygonFork() internal {
        vm.selectFork(polygonFork);
        networkConfig = polygonConfig;
        child = polygonChild;
    }
