...
type TypesChildOperationalState struct {
	Paused                    bool
	RecoveryMode              uint8
	LastHandledEpochNonce     *big.Int
	LastHandledRebalanceNonce *big.Int
	Tvl                       *big.Int
}
...
type ChildVaultInvalidEpochNonce struct {
	ProvidedNonce    *big.Int
	LastHandledNonce *big.Int
}

type ChildVaultInvalidParentChainSelector struct {
}

type ChildVaultInvalidRebalanceNonce struct {
	ProvidedNonce    *big.Int
	LastHandledNonce *big.Int
}
...
type ActiveProtocolAdapterCleared0Topics struct {
	Adapter common.Address
}

type ActiveProtocolAdapterCleared0Decoded struct {
	Adapter common.Address
}
...
type EpochDepositToStrategyFailureTopics struct {
	EpochNonce *big.Int
	Amount     *big.Int
}

type EpochDepositToStrategyFailureDecoded struct {
	EpochNonce *big.Int
	Amount     *big.Int
}
...
type EpochWithdrawFromStrategyFailureTopics struct {
	EpochNonce *big.Int
	Amount     *big.Int
}

type EpochWithdrawFromStrategyFailureDecoded struct {
	EpochNonce *big.Int
	Amount     *big.Int
}
...
type ChildVaultCodec interface {
	...
	EncodeGetChildOperationalStateMethodCall() ([]byte, error)
	DecodeGetChildOperationalStateMethodOutput(data []byte) (TypesChildOperationalState, error)
	...
	EncodeGetLastHandledEpochNonceMethodCall() ([]byte, error)
	DecodeGetLastHandledEpochNonceMethodOutput(data []byte) (*big.Int, error)
	EncodeGetLastHandledRebalanceNonceMethodCall() ([]byte, error)
	DecodeGetLastHandledRebalanceNonceMethodOutput(data []byte) (*big.Int, error)
	...
	EncodeTypesChildOperationalStateStruct(in TypesChildOperationalState) ([]byte, error)
	...
	ActiveProtocolAdapterCleared0LogHash() []byte
	EncodeActiveProtocolAdapterCleared0Topics(evt abi.Event, values []ActiveProtocolAdapterCleared0Topics) ([]*evm.TopicValues, error)
	DecodeActiveProtocolAdapterCleared0(log *evm.Log) (*ActiveProtocolAdapterCleared0Decoded, error)
	...
	EpochDepositToStrategyFailureLogHash() []byte
	EncodeEpochDepositToStrategyFailureTopics(evt abi.Event, values []EpochDepositToStrategyFailureTopics) ([]*evm.TopicValues, error)
	DecodeEpochDepositToStrategyFailure(log *evm.Log) (*EpochDepositToStrategyFailureDecoded, error)
	...
	EpochWithdrawFromStrategyFailureLogHash() []byte
	EncodeEpochWithdrawFromStrategyFailureTopics(evt abi.Event, values []EpochWithdrawFromStrategyFailureTopics) ([]*evm.TopicValues, error)
	DecodeEpochWithdrawFromStrategyFailure(log *evm.Log) (*EpochWithdrawFromStrategyFailureDecoded, error)
...
func (c *Codec) EncodeGetChildOperationalStateMethodCall() ([]byte, error) {
	return c.abi.Pack("getChildOperationalState")
}

func (c *Codec) DecodeGetChildOperationalStateMethodOutput(data []byte) (TypesChildOperationalState, error) {
	vals, err := c.abi.Methods["getChildOperationalState"].Outputs.Unpack(data)
	if err != nil {
		return *new(TypesChildOperationalState), err
	}
	jsonData, err := json.Marshal(vals[0])
	if err != nil {
		return *new(TypesChildOperationalState), fmt.Errorf("failed to marshal ABI result: %w", err)
	}

	var result TypesChildOperationalState
	if err := json.Unmarshal(jsonData, &result); err != nil {
		return *new(TypesChildOperationalState), fmt.Errorf("failed to unmarshal to TypesChildOperationalState: %w", err)
	}
...
func (c *Codec) EncodeGetLastHandledEpochNonceMethodCall() ([]byte, error) {
	return c.abi.Pack("getLastHandledEpochNonce")
}

func (c *Codec) DecodeGetLastHandledEpochNonceMethodOutput(data []byte) (*big.Int, error) {
	vals, err := c.abi.Methods["getLastHandledEpochNonce"].Outputs.Unpack(data)
	if err != nil {
		return *new(*big.Int), err
	}
	jsonData, err := json.Marshal(vals[0])
...
func (c *Codec) EncodeGetLastHandledRebalanceNonceMethodCall() ([]byte, error) {
	return c.abi.Pack("getLastHandledRebalanceNonce")
}

func (c *Codec) DecodeGetLastHandledRebalanceNonceMethodOutput(data []byte) (*big.Int, error) {
	vals, err := c.abi.Methods["getLastHandledRebalanceNonce"].Outputs.Unpack(data)
	if err != nil {
		return *new(*big.Int), err
	}
	jsonData, err := json.Marshal(vals[0])
...
func (c *Codec) EncodeTypesChildOperationalStateStruct(in TypesChildOperationalState) ([]byte, error) {
	tupleType, err := abi.NewType(
		"tuple", "",
		[]abi.ArgumentMarshaling{
			{Name: "paused", Type: "bool"},
			{Name: "recoveryMode", Type: "uint8"},
			{Name: "lastHandledEpochNonce", Type: "uint256"},
			{Name: "lastHandledRebalanceNonce", Type: "uint256"},
			{Name: "tvl", Type: "uint256"},
		},
	)
	if err != nil {
		return nil, fmt.Errorf("failed to create tuple type for TypesChildOperationalState: %w", err)
	}
	args := abi.Arguments{
		{Name: "typesChildOperationalState", Type: tupleType},
	}
...
func (c *Codec) ActiveProtocolAdapterCleared0LogHash() []byte {
	return c.abi.Events["ActiveProtocolAdapterCleared0"].ID.Bytes()
}

func (c *Codec) EncodeActiveProtocolAdapterCleared0Topics(
	evt abi.Event,
	values []ActiveProtocolAdapterCleared0Topics,
) ([]*evm.TopicValues, error) {
	var adapterRule []interface{}
	for _, v := range values {
		if reflect.ValueOf(v.Adapter).IsZero() {
...
// DecodeActiveProtocolAdapterCleared0 decodes a log into a ActiveProtocolAdapterCleared0 struct.
func (c *Codec) DecodeActiveProtocolAdapterCleared0(log *evm.Log) (*ActiveProtocolAdapterCleared0Decoded, error) {
	event := new(ActiveProtocolAdapterCleared0Decoded)
	if err := c.abi.UnpackIntoInterface(event, "ActiveProtocolAdapterCleared0", log.Data); err != nil {
		return nil, err
	}
	var indexed abi.Arguments
	for _, arg := range c.abi.Events["ActiveProtocolAdapterCleared0"].Inputs {
		if arg.Indexed {
			if arg.Type.T == abi.TupleTy {
				// abigen throws on tuple, so converting to bytes to
				// receive back the common.Hash as is instead of error
...
func (c *Codec) EpochDepositToStrategyFailureLogHash() []byte {
	return c.abi.Events["EpochDepositToStrategyFailure"].ID.Bytes()
}

func (c *Codec) EncodeEpochDepositToStrategyFailureTopics(
	evt abi.Event,
	values []EpochDepositToStrategyFailureTopics,
) ([]*evm.TopicValues, error) {
	var epochNonceRule []interface{}
	for _, v := range values {
		if reflect.ValueOf(v.EpochNonce).IsZero() {
...
// DecodeEpochDepositToStrategyFailure decodes a log into a EpochDepositToStrategyFailure struct.
func (c *Codec) DecodeEpochDepositToStrategyFailure(log *evm.Log) (*EpochDepositToStrategyFailureDecoded, error) {
	event := new(EpochDepositToStrategyFailureDecoded)
	if err := c.abi.UnpackIntoInterface(event, "EpochDepositToStrategyFailure", log.Data); err != nil {
		return nil, err
	}
	var indexed abi.Arguments
	for _, arg := range c.abi.Events["EpochDepositToStrategyFailure"].Inputs {
		if arg.Indexed {
			if arg.Type.T == abi.TupleTy {
				// abigen throws on tuple, so converting to bytes to
				// receive back the common.Hash as is instead of error
...
func (c *Codec) EpochWithdrawFromStrategyFailureLogHash() []byte {
	return c.abi.Events["EpochWithdrawFromStrategyFailure"].ID.Bytes()
}

func (c *Codec) EncodeEpochWithdrawFromStrategyFailureTopics(
	evt abi.Event,
	values []EpochWithdrawFromStrategyFailureTopics,
) ([]*evm.TopicValues, error) {
	var epochNonceRule []interface{}
	for _, v := range values {
		if reflect.ValueOf(v.EpochNonce).IsZero() {
...
// DecodeEpochWithdrawFromStrategyFailure decodes a log into a EpochWithdrawFromStrategyFailure struct.
func (c *Codec) DecodeEpochWithdrawFromStrategyFailure(log *evm.Log) (*EpochWithdrawFromStrategyFailureDecoded, error) {
	event := new(EpochWithdrawFromStrategyFailureDecoded)
	if err := c.abi.UnpackIntoInterface(event, "EpochWithdrawFromStrategyFailure", log.Data); err != nil {
		return nil, err
	}
	var indexed abi.Arguments
	for _, arg := range c.abi.Events["EpochWithdrawFromStrategyFailure"].Inputs {
		if arg.Indexed {
			if arg.Type.T == abi.TupleTy {
				// abigen throws on tuple, so converting to bytes to
				// receive back the common.Hash as is instead of error
...
func (c ChildVault) GetChildOperationalState(
	runtime cre.Runtime,
	blockNumber *big.Int,
) cre.Promise[TypesChildOperationalState] {
	calldata, err := c.Codec.EncodeGetChildOperationalStateMethodCall()
	if err != nil {
		return cre.PromiseFromResult[TypesChildOperationalState](*new(TypesChildOperationalState), err)
	}
	...
	return cre.Then(promise, func(response *evm.CallContractReply) (TypesChildOperationalState, error) {
		return c.Codec.DecodeGetChildOperationalStateMethodOutput(response.Data)
	})
...
func (c ChildVault) GetLastHandledEpochNonce(
	runtime cre.Runtime,
	blockNumber *big.Int,
) cre.Promise[*big.Int] {
	calldata, err := c.Codec.EncodeGetLastHandledEpochNonceMethodCall()
	if err != nil {
		return cre.PromiseFromResult[*big.Int](*new(*big.Int), err)
	}
	...
	return cre.Then(promise, func(response *evm.CallContractReply) (*big.Int, error) {
		return c.Codec.DecodeGetLastHandledEpochNonceMethodOutput(response.Data)
	})

}

func (c ChildVault) GetLastHandledRebalanceNonce(
	runtime cre.Runtime,
	blockNumber *big.Int,
) cre.Promise[*big.Int] {
	calldata, err := c.Codec.EncodeGetLastHandledRebalanceNonceMethodCall()
	if err != nil {
		return cre.PromiseFromResult[*big.Int](*new(*big.Int), err)
	}
	...
	return cre.Then(promise, func(response *evm.CallContractReply) (*big.Int, error) {
		return c.Codec.DecodeGetLastHandledRebalanceNonceMethodOutput(response.Data)
	})
...
func (c ChildVault) WriteReportFromTypesChildOperationalState(
	runtime cre.Runtime,
	input TypesChildOperationalState,
	gasConfig *evm.GasConfig,
) cre.Promise[*evm.WriteReportReply] {
	encoded, err := c.Codec.EncodeTypesChildOperationalStateStruct(input)
	if err != nil {
		return cre.PromiseFromResult[*evm.WriteReportReply](nil, err)
	}
	promise := runtime.GenerateReport(&pb2.ReportRequest{
...
func (c *ChildVault) DecodeBaseVaultInvalidSourceChainSelectorError(data []byte) (*BaseVaultInvalidSourceChainSelector, error) {
...
// DecodeChildVaultInvalidEpochNonceError decodes a ChildVault__InvalidEpochNonce error from revert data.
func (c *ChildVault) DecodeChildVaultInvalidEpochNonceError(data []byte) (*ChildVaultInvalidEpochNonce, error) {
	args := c.ABI.Errors["ChildVault__InvalidEpochNonce"].Inputs
	values, err := args.Unpack(data[4:])
	if err != nil {
		return nil, fmt.Errorf("failed to unpack error: %w", err)
	}
	...
	providedNonce, ok0 := values[0].(*big.Int)
	if !ok0 {
		return nil, fmt.Errorf("unexpected type for providedNonce in ChildVaultInvalidEpochNonce error")
	}

	lastHandledNonce, ok1 := values[1].(*big.Int)
	if !ok1 {
		return nil, fmt.Errorf("unexpected type for lastHandledNonce in ChildVaultInvalidEpochNonce error")
	}

	return &ChildVaultInvalidEpochNonce{
		ProvidedNonce:    providedNonce,
		LastHandledNonce: lastHandledNonce,
	}, nil
}

// Error implements the error interface for ChildVaultInvalidEpochNonce.
func (e *ChildVaultInvalidEpochNonce) Error() string {
	return fmt.Sprintf("ChildVaultInvalidEpochNonce error: providedNonce=%v; lastHandledNonce=%v;", e.ProvidedNonce, e.LastHandledNonce)
}
...
// DecodeChildVaultInvalidRebalanceNonceError decodes a ChildVault__InvalidRebalanceNonce error from revert data.
func (c *ChildVault) DecodeChildVaultInvalidRebalanceNonceError(data []byte) (*ChildVaultInvalidRebalanceNonce, error) {
	args := c.ABI.Errors["ChildVault__InvalidRebalanceNonce"].Inputs
	values, err := args.Unpack(data[4:])
	if err != nil {
		return nil, fmt.Errorf("failed to unpack error: %w", err)
	}
	...
	providedNonce, ok0 := values[0].(*big.Int)
	if !ok0 {
		return nil, fmt.Errorf("unexpected type for providedNonce in ChildVaultInvalidRebalanceNonce error")
	}

	lastHandledNonce, ok1 := values[1].(*big.Int)
	if !ok1 {
		return nil, fmt.Errorf("unexpected type for lastHandledNonce in ChildVaultInvalidRebalanceNonce error")
	}

	return &ChildVaultInvalidRebalanceNonce{
		ProvidedNonce:    providedNonce,
		LastHandledNonce: lastHandledNonce,
	}, nil
}

// Error implements the error interface for ChildVaultInvalidRebalanceNonce.
func (e *ChildVaultInvalidRebalanceNonce) Error() string {
	return fmt.Sprintf("ChildVaultInvalidRebalanceNonce error: providedNonce=%v; lastHandledNonce=%v;", e.ProvidedNonce, e.LastHandledNonce)
}
...
func (c *ChildVault) UnpackError(data []byte) (any, error) {
	...
	case common.Bytes2Hex(c.ABI.Errors["ChildVault__InvalidEpochNonce"].ID.Bytes()[:4]):
		return c.DecodeChildVaultInvalidEpochNonceError(data)
	...
	case common.Bytes2Hex(c.ABI.Errors["ChildVault__InvalidRebalanceNonce"].ID.Bytes()[:4]):
		return c.DecodeChildVaultInvalidRebalanceNonceError(data)
...
// ActiveProtocolAdapterCleared0Trigger wraps the raw log trigger and provides decoded ActiveProtocolAdapterCleared0Decoded data
type ActiveProtocolAdapterCleared0Trigger struct {
	cre.Trigger[*evm.Log, *evm.Log]             // Embed the raw trigger
	contract                        *ChildVault // Keep reference for decoding
}

// Adapt method that decodes the log into ActiveProtocolAdapterCleared0 data
func (t *ActiveProtocolAdapterCleared0Trigger) Adapt(l *evm.Log) (*bindings.DecodedLog[ActiveProtocolAdapterCleared0Decoded], error) {
	// Decode the log using the contract's codec
	decoded, err := t.contract.Codec.DecodeActiveProtocolAdapterCleared0(l)
	if err != nil {
		return nil, fmt.Errorf("failed to decode ActiveProtocolAdapterCleared0 log: %w", err)
	}

	return &bindings.DecodedLog[ActiveProtocolAdapterCleared0Decoded]{
		Log:  l,        // Original log
		Data: *decoded, // Decoded data
	}, nil
}

func (c *ChildVault) LogTriggerActiveProtocolAdapterCleared0Log(chainSelector uint64, confidence evm.ConfidenceLevel, filters []ActiveProtocolAdapterCleared0Topics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[ActiveProtocolAdapterCleared0Decoded]], error) {
	event := c.ABI.Events["ActiveProtocolAdapterCleared0"]
	topics, err := c.Codec.EncodeActiveProtocolAdapterCleared0Topics(event, filters)
	if err != nil {
		return nil, fmt.Errorf("failed to encode topics for ActiveProtocolAdapterCleared0: %w", err)
	}
	...
	return &ActiveProtocolAdapterCleared0Trigger{
		Trigger:  rawTrigger,
		contract: c,
	}, nil
}

func (c *ChildVault) FilterLogsActiveProtocolAdapterCleared0(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
		FilterQuery: &evm.FilterQuery{
			Addresses: [][]byte{c.Address.Bytes()},
			Topics: []*evm.Topics{
				{Topic: [][]byte{c.Codec.ActiveProtocolAdapterCleared0LogHash()}},
			},
			BlockHash: options.BlockHash,
			FromBlock: pb.NewBigIntFromInt(options.FromBlock),
			ToBlock:   pb.NewBigIntFromInt(options.ToBlock),
...
func (c *ChildVault) LogTriggerCCIPBridgedLog(chainSelector uint64, confidence evm.ConfidenceLevel, filters []CCIPBridgedTopics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[CCIPBridgedDecoded]], error) {
...
func (c *ChildVault) FilterLogsCCIPBridged(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
...
func (c *ChildVault) LogTriggerCCIPReceivedLog(chainSelector uint64, confidence evm.ConfidenceLevel, filters []CCIPReceivedTopics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[CCIPReceivedDecoded]], error) {
...
func (c *ChildVault) FilterLogsCCIPReceived(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
...
// EpochDepositToStrategyFailureTrigger wraps the raw log trigger and provides decoded EpochDepositToStrategyFailureDecoded data
type EpochDepositToStrategyFailureTrigger struct {
	cre.Trigger[*evm.Log, *evm.Log]             // Embed the raw trigger
	contract                        *ChildVault // Keep reference for decoding
}

// Adapt method that decodes the log into EpochDepositToStrategyFailure data
func (t *EpochDepositToStrategyFailureTrigger) Adapt(l *evm.Log) (*bindings.DecodedLog[EpochDepositToStrategyFailureDecoded], error) {
	// Decode the log using the contract's codec
	decoded, err := t.contract.Codec.DecodeEpochDepositToStrategyFailure(l)
	if err != nil {
		return nil, fmt.Errorf("failed to decode EpochDepositToStrategyFailure log: %w", err)
	}

	return &bindings.DecodedLog[EpochDepositToStrategyFailureDecoded]{
		Log:  l,        // Original log
		Data: *decoded, // Decoded data
	}, nil
}

func (c *ChildVault) LogTriggerEpochDepositToStrategyFailureLog(chainSelector uint64, confidence evm.ConfidenceLevel, filters []EpochDepositToStrategyFailureTopics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[EpochDepositToStrategyFailureDecoded]], error) {
	event := c.ABI.Events["EpochDepositToStrategyFailure"]
	topics, err := c.Codec.EncodeEpochDepositToStrategyFailureTopics(event, filters)
	if err != nil {
		return nil, fmt.Errorf("failed to encode topics for EpochDepositToStrategyFailure: %w", err)
	}
	...
	return &EpochDepositToStrategyFailureTrigger{
		Trigger:  rawTrigger,
		contract: c,
	}, nil
}

func (c *ChildVault) FilterLogsEpochDepositToStrategyFailure(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
		FilterQuery: &evm.FilterQuery{
			Addresses: [][]byte{c.Address.Bytes()},
			Topics: []*evm.Topics{
				{Topic: [][]byte{c.Codec.EpochDepositToStrategyFailureLogHash()}},
			},
			BlockHash: options.BlockHash,
			FromBlock: pb.NewBigIntFromInt(options.FromBlock),
			ToBlock:   pb.NewBigIntFromInt(options.ToBlock),
...
func (c *ChildVault) LogTriggerEpochDepositToStrategySuccessLog(chainSelector uint64, confidence evm.ConfidenceLevel, filters []EpochDepositToStrategySuccessTopics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[EpochDepositToStrategySuccessDecoded]], error) {
...
func (c *ChildVault) FilterLogsEpochDepositToStrategySuccess(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
...
// EpochWithdrawFromStrategyFailureTrigger wraps the raw log trigger and provides decoded EpochWithdrawFromStrategyFailureDecoded data
type EpochWithdrawFromStrategyFailureTrigger struct {
	cre.Trigger[*evm.Log, *evm.Log]             // Embed the raw trigger
	contract                        *ChildVault // Keep reference for decoding
}

// Adapt method that decodes the log into EpochWithdrawFromStrategyFailure data
func (t *EpochWithdrawFromStrategyFailureTrigger) Adapt(l *evm.Log) (*bindings.DecodedLog[EpochWithdrawFromStrategyFailureDecoded], error) {
	// Decode the log using the contract's codec
	decoded, err := t.contract.Codec.DecodeEpochWithdrawFromStrategyFailure(l)
	if err != nil {
		return nil, fmt.Errorf("failed to decode EpochWithdrawFromStrategyFailure log: %w", err)
	}

	return &bindings.DecodedLog[EpochWithdrawFromStrategyFailureDecoded]{
		Log:  l,        // Original log
		Data: *decoded, // Decoded data
	}, nil
}

func (c *ChildVault) LogTriggerEpochWithdrawFromStrategyFailureLog(chainSelector uint64, confidence evm.ConfidenceLevel, filters []EpochWithdrawFromStrategyFailureTopics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[EpochWithdrawFromStrategyFailureDecoded]], error) {
	event := c.ABI.Events["EpochWithdrawFromStrategyFailure"]
	topics, err := c.Codec.EncodeEpochWithdrawFromStrategyFailureTopics(event, filters)
	if err != nil {
		return nil, fmt.Errorf("failed to encode topics for EpochWithdrawFromStrategyFailure: %w", err)
	}
	...
	return &EpochWithdrawFromStrategyFailureTrigger{
		Trigger:  rawTrigger,
		contract: c,
	}, nil
}

func (c *ChildVault) FilterLogsEpochWithdrawFromStrategyFailure(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
		FilterQuery: &evm.FilterQuery{
			Addresses: [][]byte{c.Address.Bytes()},
			Topics: []*evm.Topics{
				{Topic: [][]byte{c.Codec.EpochWithdrawFromStrategyFailureLogHash()}},
			},
			BlockHash: options.BlockHash,
			FromBlock: pb.NewBigIntFromInt(options.FromBlock),
			ToBlock:   pb.NewBigIntFromInt(options.ToBlock),
...
func (c *ChildVault) LogTriggerEpochWithdrawFromStrategySuccessLog(chainSelector uint64, confidence evm.ConfidenceLevel, filters []EpochWithdrawFromStrategySuccessTopics) (cre.Trigger[*evm.Log, *bindings.DecodedLog[EpochWithdrawFromStrategySuccessDecoded]], error) {
...
func (c *ChildVault) FilterLogsEpochWithdrawFromStrategySuccess(runtime cre.Runtime, options *bindings.FilterOptions) (cre.Promise[*evm.FilterLogsReply], error) {
	if options == nil {
		return nil, errors.New("FilterLogs options are required.")
	}
	return c.client.FilterLogs(runtime, &evm.FilterLogsRequest{
