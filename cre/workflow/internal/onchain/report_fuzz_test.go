...
func Fuzz_SubmitReport_propagatesWriteInputs(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, calldata []byte, gasLimit uint64, router string) {
		...
		err := SubmitReport(runtime, helper.EvmConfig{ChainSelector: 123, WorkflowRouterAddress: router, GasLimit: gasLimit}, 0, calldata)
		if workflowRouter == (common.Address{}) || gasLimit == 0 || gasLimit > 5_000_000 || len(calldata) < 4 || len(calldata)+60 > 5*1024 {
			require.Error(t, err)
		...
		require.Equal(t, calldata, runtime.reportRequest.EncodedPayload[60:], "unexpected report payload")
