...
| `CCIP-005a` | After a failed Child outbound send, `s_ccipSendRecovery` exactly records the intended tx type, destination chain, amount, tx data, and a nonzero creation timestamp.                                      | `postcondition` | implemented: Foundry |
| `CCIP-005b` | While `s_ccipSendRecovery.amount != 0`, `USDC.balanceOf(ChildVault) >= s_ccipSendRecovery.amount`. See `SOLV-004`.                                                                                    | `invariant`     | implemented: Foundry |
| `CCIP-005c` | A successful `recoverFailedCcipSend` replays the stored send and completes the intended parent-side state transition for the stored message.                                                            | `postcondition` | implemented: Foundry |
...
| `REC-003`  | Successful recovery clears state.                                                                                                                                                                                                             | `postcondition`  | implemented: Foundry |
