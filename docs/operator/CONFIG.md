...
Vault pause state blocks normal user, epoch, rebalance, recovery, inbound CCIP, and ParentVault completion flows. In particular, `completeEpochDeposit(expectedEpochNonce, actualDepositAmount)` and `completeRebalance(expectedRebalanceNonce)` cannot acknowledge CRE-observed remote success while ParentVault is paused. After reconciling cross-chain state, operators must use the controlled unpause procedure in [OPERATIONS](./OPERATIONS.md#paused-cross-chain-execution) to resume or finalize an in-flight operation. Each completion uses the nonce from its corresponding destination success event; epoch-deposit completion also uses the amount from `EpochDepositToStrategySuccess`.
