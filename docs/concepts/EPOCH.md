...
If the active strategy is local to the parent chain, settlement can complete synchronously. If the active strategy is on a child chain, a remote net deposit remains executing until the ChildVault strategy deposit succeeds and CRE calls `completeEpochDeposit(expectedEpochNonce, actualDepositAmount)` with the epoch nonce and amount emitted by `EpochDepositToStrategySuccess`. A remote net-withdraw shortfall below `getRemoteWithdrawDustThreshold()` is terminal dust: ParentVault sends no Child request, makes the epoch immediately claimable, and funds its withdraw claim pool only with deposits already held on Parent. A remote net withdrawal at or above the threshold remains executing until the child withdrawal and CCIP return complete, after which ParentVault allocates the complete amount delivered through CCIP to withdraw claims. The threshold is one hundredth of a whole underlying asset unit (`0.01 USDC` for the USDC vault).
