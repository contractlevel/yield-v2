...
If the active strategy is local to the parent chain, settlement can complete synchronously. If the active strategy is on a child chain, a remote net deposit remains executing until the ChildVault strategy deposit succeeds and CRE calls `completeEpochDeposit(expectedEpochNonce, actualDepositAmount)` with the epoch nonce and amount emitted by `EpochDepositToStrategySuccess`. A remote net-withdraw shortfall at or below `getMinAssetAmount()` is terminal dust: ParentVault sends no Child request, makes the epoch immediately claimable, and funds its withdraw claim pool only with deposits already held on Parent. A larger remote net withdrawal remains executing until the child withdrawal and CCIP return complete. ParentVault then transfers `min(receivedAmount, getMinAssetAmount())` of the returned asset to the treasury and allocates the remainder to withdraw claims.
...
- `totalWithdrawClaimAmount` — the total underlying asset available for withdraw claims. Remote dust uses only Parent-held deposits. For a serviced remote net withdrawal, the value recorded at epoch close is provisional and is replaced with Parent-held deposits plus the actual amount returned through CCIP, less the settlement charge, before the epoch becomes `CLAIMABLE`.
