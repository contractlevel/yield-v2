...
If the active strategy is local to the parent chain, settlement can complete synchronously. If the active strategy is on a child chain, a remote net deposit remains executing until the ChildVault strategy deposit succeeds and CRE calls `completeEpochDeposit(expectedEpochNonce, actualDepositAmount)` with the epoch nonce and amount emitted by `EpochDepositToStrategySuccess`. A remote net withdrawal below `getMinAssetAmount()` defers settlement: `closeEpoch` reverts atomically and the epoch remains open until deposits, additional withdrawals, or cancellation move its aggregate net flow out of that range. A remote net withdrawal at or above the minimum remains executing until the child withdrawal and CCIP return complete, after which ParentVault allocates the complete amount delivered through CCIP to withdrawal claims. The minimum is one whole underlying asset unit (`1 USDC` for the USDC vault).
