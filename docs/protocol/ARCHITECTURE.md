...
- **WorkflowRouter** — validates CRE workflow identity, the signed target chain and router, report freshness, and allowlisted selectors, then strips the report envelope and forwards its vault calldata to the immutable vault.
...
1. **Deposit** — a user calls `ParentVault.deposit`, or a payer calls `depositFor` to fund a position
   owned by a beneficiary. Funds are escrowed for the current epoch; no shares are minted yet.
2. **Epoch close** — CRE triggers `closeEpoch`. The vault uses the supplied TVL to calculate deposit-share allocations and withdrawal-asset entitlements directly, then updates per-epoch accounting; shares are not minted at close. A depositor can call `claimShares`, or any caller can use `claimSharesFor` to process the claim while sending shares only to the depositor.
...
4. **Withdraw** — a user calls `withdraw`, or a payer calls `withdrawFor` to fund an intent owned by
   a beneficiary. On settlement, the underlying amount is reserved. Escrowed shares are burned and
   assets are paid to the position owner through `claimAsset` or permissionless `claimAssetFor`.
